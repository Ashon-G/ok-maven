...
export const VelocityHero = () => {
  ...
  return (
    <section ref={targetRef} className="h-[500vh] bg-white text-black">
      <div className="sticky top-0 flex h-screen flex-col justify-between overflow-hidden">
        <Nav isMenuOpen={isMenuOpen} setIsMenuOpen={setIsMenuOpen} />
        <CenterCopy />
        <motion.p
          style={{ skewX, x }}
          className="origin-bottom-left whitespace-nowrap text-7xl font-black uppercase leading-[0.85] text-black/20 md:text-9xl md:leading-[0.85]"
        >
...
const Nav = ({ isMenuOpen, setIsMenuOpen }: { isMenuOpen: boolean; setIsMenuOpen: (value: boolean) => void }) => {
  return (
    <div className="relative mb-1 flex w-full justify-between p-6">
      ...
      <div className={`
        fixed inset-0 bg-white transform transition-transform duration-300 ease-in-out md:relative md:inset-auto md:bg-transparent md:transform-none
...
const Logo = () => {
  return (
    <div className="text-2xl font-bold text-black z-50">
...
const Links = ({ setIsMenuOpen }: { setIsMenuOpen: (value: boolean) => void }) => {
  ...
  return (
    <nav className="flex flex-col md:flex-row items-center justify-center h-full md:h-auto gap-8 md:gap-6 text-lg md:text-sm">
      <a href="#how-it-works" onClick={handleClick} className="hover:text-secondary text-black">How it Works</a>
      <a href="#pricing" onClick={handleClick} className="hover:text-secondary text-black">Pricing</a>
      <a href="#contact" onClick={handleClick} className="hover:text-secondary text-black">Contact</a>
      <div className="flex flex-col md:flex-row items-center gap-4 md:ml-4">
        <Link
          to="/login"
          onClick={handleClick}
          className="flex items-center gap-2 rounded-lg border border-black/20 px-4 py-2 text-sm font-medium text-black transition-all hover:border-black/40 hover:text-secondary"
        >
  ...
  return (
    <div className="flex items-center justify-center px-4 text-center">
      <div className="max-w-4xl">
        <h1 className="mb-6 text-4xl font-bold sm:text-6xl md:text-7xl text-black">
          Connect with Student Talent to{" "}
          <span className="gradient-text font-black">Build Your Next Big Thing</span>
        </h1>
        <p className="mb-8 text-lg text-black/60 md:text-xl">
...
const ScrollArrow = () => {
  return (
    <>
      <div className="absolute left-4 top-1/2 hidden -translate-y-1/2 text-xs text-black/40 lg:block">
        <span style={{ writingMode: "vertical-lr" }}>SCROLL</span>
        <FiArrowDown className="mx-auto mt-2" />
      </div>
      <div className="absolute right-4 top-1/2 hidden -translate-y-1/2 text-xs text-black/40 lg:block">
        <span style={{ writingMode: "vertical-lr" }}>SCROLL</span>
        <FiArrowDown className="mx-auto mt-2" />
      </div>
    </>
